(function (factory) {
    ...
    // 验证是否引用jquery
    if (!$ || !$.fn || !$.fn.jquery) {
    	BJUI.alertmsg('warn', '在引用wangEditor.js之前，先引用jQuery，否则无法使用 wangEditor');
        return;
    }
...
(function (window, $) {
    if (window.wangEditor) {
        // 重复引用
    	BJUI.alertmsg('warn', '一个页面不能重复引用 wangEditor.js 或 wangEditor.min.js ！！！');
        return;
    }
...
_e(function (E, $) {
    ...
    E.config.emotions = {
         'default': {
             title: '默认',
             data: 'B-JUI/plugins/wangEditor/js/emotions.data'
         },
        'weibo': {
            title: '微博表情',
            data: [
    ...
    // 百度地图的key
    E.config.mapAk = 'BdaYfrLOtlLrWbxGyxdiYUBQmjOoIwyQ';
...
_e(function (E, $) {
     ...
     E.UI.menus = {
        ...
        fontfamily: {
            normal: '<a href="#" tabindex="-1"><i class="wangeditor-menu-img-font"></i></a>',
            selected: '.selected'
        },
        fontsize: {
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if (!reg.test(link)) {
            	BJUI.alertmsg('info','视频链接格式错误！');
                menu.dropPanel.focusFirstInput();
                return;
            }

            if (isNaN(width) || isNaN(height)) {
            	BJUI.alertmsg('info','宽度或高度不是数字！');
                return;
            }
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if(isDynamic){
                if(markers.length > 1){
                	BJUI.alertmsg('error', lang.langDynamicOneLocation );
                    return;
                }
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            if (resultText.indexOf('error|') === 0) {
                ...
                BJUI.alertmsg('info', resultText.split('|')[1]);
            } else {
        ...
        fns.ontimeout || (fns.ontimeout = function (xhr) {
            E.error('上传图片超时');
            BJUI.alertmsg('info','上传图片超时');
        });

        // -------- 定义error函数 --------
        fns.onerror || (fns.onerror = function (xhr) {
            E.error('上传上图片发生错误');
            BJUI.alertmsg('info','上传上图片发生错误');
        });
        ...
        reader.onload = function (e) {
            ...
            editor.xhrUploadImg({
                ...
                errorfn: function (xhr) {
                    clearInput();
                    if (E.isOnWebsite) {
                    	BJUI.alertmsg('error','wangEditor官网暂时没有服务端，因此报错。实际项目中不会发生');
                    }
                    // 执行配置中的方法
                    var editor = this;
                ...
                timeoutfn: function (xhr) {
                    clearInput();
                    if (E.isOnWebsite) {
                    	BJUI.alertmsg('error','wangEditor官网暂时没有服务端，因此超时。实际项目中不会发生');
                    }
                    // 执行配置中的方法
                    var editor = this;
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        if (!customUpload) {
        	...
        	BJUI.alertmsg('warn','自定义上传无效，详看浏览器日志console.log');
