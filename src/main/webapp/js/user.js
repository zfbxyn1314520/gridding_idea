...
function addUserInfo() {
    BJUI.dialog({
        ...
        onLoad: function ($dialog) {
            ...
            var wW = $(window).width(), iTop = that.options.max ? 0 : (($(window).height() - 550) / 3);
            ...
            $("#headIcon").click(function (event) {
                $dialog.height(550)
                    ...
                    .find('> .dialogContent').height(550 - $('> .dialogHeader', $dialog).outerHeight());
...
function editUserInfo() {
    ...
    if (typeof(row) == "undefined" || row.length != 1) {
        ...
        if (row[0].role.roleLevel > 5) {
            BJUI.alertmsg('confirm', '请在&emsp;<span style="color: orange">' +
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].roleName == '其他' || data[i].roleLevel > 5) {
                    continue;
                } else {
